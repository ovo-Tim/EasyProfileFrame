...
class ProfileFrameObject:
    def __init__(self, obj: Feature):
        ...
        # Add properties
        obj.addProperty(
            "App::PropertyString", "Sketch", "EasyProfileFrame", "Sweep sketch"
        ).Sketch = ""
        obj.addProperty(
            "App::PropertyString",
            "EdgeName",
            "EasyProfileFrame",
            "Edge reference (Sketch:Edge)",
        ).EdgeName = ""
        obj.addProperty(
            "App::PropertyLength",
            "OffsetX",
        ...
        obj.addProperty(
            ...
            "Length",
            "EasyProfileFrame",
            "This property isn't included chamfer",
            read_only=True,
        ).Length = 0
        obj.addProperty(
            "App::PropertyAngle",
            "ChamferAngleL",
            "EasyProfileFrame",
            "Chamfer angle of the left side. Can only be multiples of 90",
        ).ChamferAngleL = 0
        obj.addProperty(
            "App::PropertyLength",
            "ChamferLengthL",
            "EasyProfileFrame",
            "Chamfer length of the left side",
            read_only=True,
        ).ChamferLengthL = 0.0
        obj.addProperty(
            "App::PropertyInteger",
            "ChamferDirectionL",
            "EasyProfileFrame",
            "Chamfer direction of the left side, 1~4",
        ).ChamferDirectionL = 1
        obj.addProperty(
            "App::PropertyLength",
            "ExtendedLengthL",
            "EasyProfileFrame",
            "Extended length of the left side. \
                        (When Chanfer Angle is set, this property will be determined autmaticallyy)",
        ).ExtendedLengthL = 0.0
        ...
        obj.addProperty(
            ...
            "ChamferAngleR",
            "EasyProfileFrame",
            "Chamfer angle of the right side. Can only be multiples of 90",
        ).ChamferAngleR = 0
        obj.addProperty(
            "App::PropertyLength",
            "ChamferLengthR",
            "EasyProfileFrame",
            "Chamfer length of the right side",
            read_only=True,
        ).ChamferLengthR = 0.0
        obj.addProperty(
            "App::PropertyInteger",
            "ChamferDirectionR",
            "EasyProfileFrame",
            "Chamfer direction of the right side, 1~4",
        ).ChamferDirectionR = 1
        obj.addProperty(
            "App::PropertyLength",
            "ExtendedLengthR",
            "EasyProfileFrame",
            "Extended length of the right side. \
                        (When Chanfer Angle is set, this property will be determined automatically)",
        ).ExtendedLengthR = 0.0
    ...
    def apply_offset_and_rotation(
        self, obj, offset_x, offset_y, angle, offset_z=FCUnits.Quantity(0)
    ):
        """Apply offset and rotation to the sketch"""
        # Check if OffsetX, OffsetY, or Angle has changed
        if (
            offset_x == self._last_offset_x
            and offset_y == self._last_offset_y
            and angle == self._last_angle
            and offset_z == self._last_offset_z
        ):
            # print("No change")
            return
    ...
    def execute(self, obj):
        ...
        # Chamfer
        if obj.ChamferAngleL > 0:
            baseObj, extended_length = self.create_chamfer(
                sketchL,
                obj.ChamferAngleL,
                obj.ChamferDirectionL,
                obj,
                baseObj,
                f"Chamfer_{obj.Name}_L",
            )
        ...
        if obj.ChamferAngleR > 0:
            ...
            baseObj, extended_length = self.create_chamfer(
                sketchR,
                obj.ChamferAngleR,
                obj.ChamferDirectionR,
                obj,
                baseObj,
                f"Chamfer_{obj.Name}_R",
                offset=-pad_length,
                right=True,
            )
        ...
        obj.Length = FCUnits.Quantity(
            pad_length + obj.ExtendedLengthL.Value + obj.ExtendedLengthR.Value
        )

        obj.Shape = baseObj.Shape

        # Apply offset and rotation to the obj
        self.apply_offset_and_rotation(
            obj, obj.OffsetX, obj.OffsetY, obj.Angle, offset_z=_offset_z
        )
    ...
    def pad(
        ...
        if (
            ...
            or pad_obj.Length != length
            or pad_obj.Reversed != reversed
            or pad_obj.BaseFeature != baseFeature
        ):
            pad_obj.Profile = sketch
    ...
    def __getstate__(self):
        state = {
            "_last_offset_x": self._last_offset_x.toStr(),
    ...
    def create_chamfer(
        ...
        extended_obj = self.pad(
            sketch,
            extended_length,
            body,
            f"Chamfer_extend_{name}",
            baseFeature=baseFeature,
            reversed=right,
        )

        # Draw a cutting sketch
        chamfer_sketch: SketchObject = GetExistent(
            f"chamferCuttingSketch_{name}", "Sketcher::SketchObject", body
        )
        if self.chamfer_sketch_cache[right] != (
            extended_length,
            width,
            baseFeature.Name,
            direction,
            ...
            print(
                f"{self.chamfer_sketch_cache[right]} != {(extended_length, width, baseFeature.Name, direction)}, redrawing"
            )
            if self.chamfer_sketch_cache[right] is not None:
                chamfer_sketch.Geometry = []
                chamfer_sketch.Constraints = []
            self.draw_chamfer_sketch(
                chamfer_sketch,
                extended_length,
                width,
                baseFeature,
            ...
            self.chamfer_sketch_cache[right] = (
                extended_length,
                width,
                baseFeature.Name,
                direction,
        ...
        if (
            ...
            or pocket_obj.Profile != chamfer_sketch
        ):
            pocket_obj.BaseFeature = extended_obj
    ...
    def draw_chamfer_sketch(
        self,
        chamfer_sketch: SketchObject,
        length: float,
        width: float,
        baseFeature,
        direction,
        ...
        chamfer_sketch.AttachmentOffset = App.Placement(
            App.Vector(0, 0, offset), App.Rotation(0, 90, direction * 90)
        )
        ...
        triangle = (
            [(-tx, ty), (-tx, -ty), (tx, ty)]
            if not right
            else [(tx, ty), (tx, -ty), (-tx, ty)]
        )
        chamfer_sketch.addGeometry(
            Part.LineSegment(
                App.Vector(triangle[0][0], triangle[0][1], 0),
                App.Vector(triangle[1][0], triangle[1][1], 0),
            ),
            False,
        )
        chamfer_sketch.addGeometry(
            Part.LineSegment(
                App.Vector(triangle[1][0], triangle[1][1], 0),
                App.Vector(triangle[2][0], triangle[2][1], 0),
            ),
            False,
        )
        chamfer_sketch.addGeometry(
...
def CreateProfileFrameBody(
    Sketch: SketchObject,
    EdgeName: str,
    doc: App.Document | AppPart = None,
    name="ProfileFrameBody",
):
